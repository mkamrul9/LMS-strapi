...
export default function ContentManagerCoursesPage() {
  ...
  const filtered = courses.filter((c: any) => {
    ...
    const description = c.description || c.attributes?.description || '';
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((course: any) => {
              // Support both V4 (attributes) and V5 (flat)
              ...
              const instructorName = course.instructor?.username || course.attributes?.instructor?.data?.attributes?.username || 'Senior Instructor';
                ...
                <div key={course.id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-xs hover:shadow-lg transition-all flex flex-col justify-between">
                  <div>
                    ...
                    <div className="p-6 space-y-3">
                      ...
                      <h3 className="text-base font-bold text-slate-900 line-clamp-2">{title}</h3>
                      <p className="text-xs text-slate-500 line-clamp-3 leading-relaxed">{description}</p>
                    </div>
                  </div>

                  <div className="p-6 pt-0 border-t border-slate-100 mt-4 space-y-2">
                    <Link
                      href={`/instructor/courses/${courseNavId}`}
                      className="w-full bg-slate-900 hover:bg-slate-800 text-white py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-colors"
                    >
                      <span>Manage Course</span>
                      <ExternalLink className="w-3.5 h-3.5" />
                    </Link>
                    <Link
                      href={`/instructor/courses/${courseNavId}/quizzes/new`}
                      className="w-full bg-purple-50 hover:bg-purple-100 text-purple-700 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-colors border border-purple-200"
                    >
                      <span>+ Add Quiz</span>
                    </Link>
                  </div>
